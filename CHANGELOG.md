...
**Fix: a dead ServiceNow session no longer looks like an empty instance:** The `agentQueryRecords` reply carries no success envelope, so the code read a *failed* request as an *empty* one. With an expired `g_ck` the browser replies `{ error: 'User is not authenticated' }` and `refresh_scope` answered `status: "success", tablesRefreshed: 0` — indistinguishable from a genuinely empty scope, and `query_records` likewise returned `count: 0` instead of failing. Both now surface `E_TOKEN_EXPIRED` with the real message, which is accurate and actionable: re-run `/token`.

**The headless server asks for a fresh session token when the helper tab connects:** `_settings.json`'s `g_ck` only refreshes when the helper tab relays a message for that instance, and merely connecting doesn't do it — so a server restarted after the browser session aged out began life holding a token ServiceNow would reject. It now runs `/token` for each known instance on the first helper-tab connection, which is the trigger that makes the tab hand over a new one. It's a browser-extension action, so it works even while ServiceNow is rejecting everything else.

**The headless server's traffic log says what's actually happening:** Every line now carries its subject and its outcome instead of just an action name and a 50-character correlation id. Outbound WebSocket calls name the table and the row window (`sys_ux_event  limit=200`) plus the encoded query; responses report the rows returned, echo the table, and time the round trip (`sys_ux_event  274 rows  0.4s`). HTTP request lines include the command's parameters (`refresh_scope  surfcddev  scopeName=x_snc_adptn`) and response lines a result summary (`ok  33 tables, 2542 files  9.0s`), including a loud `TRUNCATED` when a sweep hit a safety ceiling. Correlation ids are shortened to the part that correlates (`#a9b6c1·15`), timestamps and columns line up across HTTP, WebSocket and progress lines, and colour is still dropped automatically when piped or under `NO_COLOR`. Previously a scope refresh scrolled past as ~30 identical `agentQueryRecords` lines with no indication of which tables were involved or what came back.
