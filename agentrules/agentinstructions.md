<!-- SN-SCRIPTSYNC:BEGIN apiVersion=18 -->
<!-- apiVersion: 18 -->
...
1. Read `port`, `token`, `pid` from `.sn-scriptsync/agent-port.json`. Don't know which folder to
   look in? Check `~/.sn-scriptsync/servers.json` first — a per-machine index of every running
   server and the instances each one knows about (see "Finding the right server" in the
   `snu-agent-api` skill).
...
| `refresh_scope` | Reset local files in a scope back to the instance's current values — a drift-guard reset ritual. |
