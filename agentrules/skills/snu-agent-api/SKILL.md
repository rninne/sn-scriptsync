...
- Scope listing is capped at 2000 records and each table's field fetch at 200 records. If either cap is hit, the response includes `scopeListingTruncated: true` and/or a `truncatedTables` array — the scope is larger than this command currently paginates, so treat the refresh as partial and use `query_records`/`get_record` to fill in what's missing.
